...
function DeadlineNode({ data, selected }) {
  ...
  return (
    <div className={`canvas-node-card min-w-52 rounded-2xl border bg-bg-surface p-4 shadow-xl transition ${completed ? 'opacity-80 ring-2 ring-node-resource/25' : ''} ${selected ? 'border-node-task ring-2 ring-node-task/25 shadow-[0_0_30px_rgba(1,111,185,.22)]' : 'border-node-deadline/40'}`}>
